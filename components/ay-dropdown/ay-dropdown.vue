		...
		<view class="filter-wrapper" :style="{ height: height + 'rpx', top: top,'border-top':border?'1rpx solid #f2f2f2':'none' }"
		 @touchmove.stop.prevent="discard">
			<view class="inner-wrapper">
				...
				<view class="navs" :style="style_navbg_Color">
					<view class="c-flex-aligncenter" :class="{ 'c-flex-center': index > 0, actNav: index === actNav }" v-for="(item, index) in navData"
					 :style="index === actNav ? style_themeColor:''" :key="index" @click="navClick(index)">
				...
				<scroll-view scroll-y="true" class="popup" :class="popupShow ? 'popupShow' : ''">
					<view>
						<view class="item-opt c-flex-align" 
	...
	export default {
		...
		computed: {
			...
			style_navbg_Color() {
				...
				if (that.navbgColor.length > 0) {
					style = `background-color:${that.navbgColor};`;
				}
